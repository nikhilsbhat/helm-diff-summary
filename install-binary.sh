...
function download_plugin() {
  ...
  OUTPUT_BASENAME=helm-diff-summary
  version=$(grep version "$HELM_PLUGIN_DIR/plugin.yaml" | cut -d'"' -f2)

  DOWNLOAD_URL="https://github.com/nikhilsbhat/helm-diff-summary/releases/download/v$version/helm-diff-summary_${version}_${osName}_${osArch}.tar.gz"
...
function install_plugin() {
  ...
  local PROJECT_NAME="helm-diff-summary"
  ...
  mv "${HELM_PLUGIN_TEMP_PATH}"/helm-diff-summary "${HELM_PLUGIN_DIR}/bin/helm-diff-summary"
  rm -rf "${HELM_PLUGIN_TEMP_PATH}"
  rm -rf "${HELM_PLUGIN_ARTIFACT_PATH}"
}

function install() {
  echo "Installing helm-diff-summary..."
  ...
  if [ "$INSTALL_PLUGIN_STAT" != "0" ]; then
    echo "installing helm plugin helm-diff-summary failed with error code: ${INSTALL_PLUGIN_STAT}"
    exit 1
  fi

  echo
  echo "helm-diff-summary is installed."
  echo
  "${HELM_PLUGIN_DIR}"/bin/helm-diff-summary -h
  echo
  echo "See https://github.com/nikhilsbhat/helm-diff-summary#readme for more information on getting started."
}
